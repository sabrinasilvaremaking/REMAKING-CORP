...
export default function Hero() {
  return (
    <section className="relative min-h-[100svh] flex items-center justify-center overflow-hidden pt-24 pb-16 sm:pt-20 md:pb-0 bg-[#F8FAFC]">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-5 sm:px-6 md:px-12 flex flex-col items-center text-center">
        <motion.div
          initial={{ opacity: 0, y: 20, scale: 0.95 }}
          animate={{ opacity: 1, y: 0, scale: 1 }}
          transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
          className="inline-flex items-center space-x-1.5 md:space-x-2 bg-white/80 backdrop-blur-md border border-slate-200 px-3.5 py-1 md:px-4 md:py-1.5 rounded-full mb-6 md:mb-8 shadow-sm"
        >
          <Sparkles className="w-3.5 h-3.5 md:w-4 md:h-4 text-slate-800" />
          <span className="text-[11px] md:text-sm font-medium text-slate-600 tracking-wide md:tracking-wide uppercase">O FUTURO DA TECNOLOGIA NA EUROPA</span>
        </motion.div>
        ...
        <motion.h1
          ...
          className="font-display text-[42px] leading-[1.05] sm:text-6xl md:text-7xl lg:text-8xl font-bold tracking-tighter md:leading-[0.95] mb-5 md:mb-8 text-transparent bg-clip-text bg-gradient-to-b from-slate-500 via-slate-700 to-slate-800"
        >
        ...
        <motion.p
          ...
          className="text-[16px] sm:text-lg md:text-xl text-slate-600 max-w-[280px] sm:max-w-xl md:max-w-2xl mb-8 md:mb-12 font-light leading-[1.6] md:leading-relaxed"
        >
        ...
        <motion.div
          ...
          className="flex flex-col sm:flex-row items-center w-full sm:w-auto space-y-3 sm:space-y-0 sm:space-x-6"
        >
          <a
            href="#contact"
